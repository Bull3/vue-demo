      ...
      <el-tab-pane label="基本" name="first">
        <el-form ref="baseForm" key="baseForm" :model="baseForm" label-width="100px">
          <el-row>
            ...
            <el-col :md="24" :lg="12">
              <el-form-item label="上级分组" prop="account">
                <el-select v-model="baseForm.account" placeholder="请选择" class="input-max-w">
                  <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
                  </el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="继承子组权限" prop="auth">
                  <el-checkbox v-model="baseForm.auth"></el-checkbox>
              </el-form-item>
              <el-form-item label="说明" prop="account">
...
export default {
  data() {
    return {
      ...
      btnData2: [
        {
          btns: [
            ...
            { code: "setUserGroup", name: "设置用户", notDialog: true },
          ]
        }
      ],
      btnData3: [
      ...
      columns: [
        { prop: "uname", label: "用户编码" },
        { prop: "remark", label: "用户名称" },
      ],
      tableData: [
