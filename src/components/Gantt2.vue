<template>
  <div ref="gantt_here" v-bind="$attrs"></div>
</template>

<script>
import "./dhtmlxGantt/analytics.js";
import "./dhtmlxGantt/dhtmlxgantt.min.js";
import "./dhtmlxGantt/ext/dhtmlxgantt_grouping.js";
import "./dhtmlxGantt/ext/dhtmlxgantt_undo.js";
// import "./dhtmlxGantt/ext/dhtmlxgantt_auto_scheduling.js";
// import "./dhtmlxGantt/ext/resource_filter.js";
  ...
  mounted() {
      ...
      if (store.hasChild(resourceId)) {
        ...
        store.getChildren(resourceId).forEach(function(childId) {
          assignments = assignments.concat(
            gantt.getResourceAssignments(childId)
          );
        });
      } else {
      ...
      columns: [
...

        {
          ...
          template: function(resource) {
            ...
            return (
              gantt.calculateDuration(state.min_date, state.max_date) * n * WORK_DAY + "h"
            );
          }
        }
      ]
      ...
      { name: "start_date", align: "center", width: 80, resize: true },
      {
        ...
        template: function(task) {
          ...
          if (assignments.length == 1) {
            return store.getItem(assignments[0].resource_id) ? store.getItem(assignments[0].resource_id).text : '';
          }
    ...
    gantt.attachEvent("onTaskLoading", function(task) {
      ...
      if (
        ...
        task[gantt.config.resource_property] = [{ resource_id: task.owner_id, value: 0 }]; //'张三' group
      }
      return true;
    });
    // gantt.load("../common/resource_project_assignments.json");
    var myJson = {
      data: [
        {
          id: 1,
    ...
    gantt.$resourcesStore.parse([
      { id: 1, text: "前端开发", parent: null },
      { id: 2, text: "张三", parent: 1 },
      { id: 3, text: "李四", parent: 1 },
    ]);
    gantt._process_loading(myJson);
  }
};
</script>
...
@import "./dhtmlxGantt/controls_styles.css";
</style>
<style>
html,
body {
